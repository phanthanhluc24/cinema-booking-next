...
export default function Movies() {
  ...
  return (
    <div className="bg-slate-900 min-h-screen">
      <h4 className="text-white ml-4 pt-4 text-2xl">
        Top Rate <span className="text-yellow-500">&#9733;</span>{" "}
      </h4>
            ...
            {movies.map((data, index) => (
              <div className="overflow-hidden rounded-lg" key={index}>
                <Link href={`/movies/detail/${data._id}`}>
                  <img
                    className="rounded-lg w-52 h-64 transform scale-100 hover:scale-110 transition-transform duration-300 ease-in-out object-cover"
                    src={data.image}
                    alt=""
                  />
                </Link>
            ...
            {upcoming.length >= 1 ? (
              upcoming.map((data, index) => (
                <div className="flex flex-col gap-3 pl-5">
                  <img
                    className="w-72 rounded hover:cursor-pointer"
                    src={data.image}
                    alt=""
                  />
              ...
              <div className="block pl-5">
                <img 
                  className="w-72 rounded-lg shadow-lg transform scale-100 hover:scale-110 transition-transform duration-300 ease-in-out object-cover" 
                  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSJ07n5nPAV6lIMsOk0ABmjNdY9ZyTZe7-tGw&usqp=CAU"
                  alt=""
                />
